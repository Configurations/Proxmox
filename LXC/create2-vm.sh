...
#   - initialise un nouveau cluster Docker Swarm     (--initswarm)
#   - rejoint un cluster Swarm existant              (--joinswarm <MANAGER_CTID>)
#
...
#
# Difference vs create-vm.sh :
#   - --init-swarm  -> --initswarm   (sans tiret interne)
#   - --join-swarm  -> --joinswarm <MANAGER_CTID>
#         Le MANAGER_CTID est l'identifiant (CTID LXC ou VMID) du manager
#         existant. Le script resout AUTOMATIQUEMENT manager_ip + token en
#         lisant le fichier de tokens cree par --initswarm sur ce manager :
#           /root/.ssh/lxc-keys/swarm-tokens-<CTID>.json   (manager LXC)
#           /root/.ssh/vm-keys/swarm-tokens-vm<VMID>.json  (manager VM)
#
...
#   --initswarm                    Init un cluster Swarm (implique --docker + --swarm)
#   --joinswarm <MANAGER_CTID>     Rejoint un cluster (implique --docker + --swarm)
#       --as-manager               Rejoindre comme manager (defaut : worker)
#
# --initswarm et --joinswarm sont mutuellement exclusifs (erreur si combines).
# Sans aucun flag : VM nue (pas de Docker, pas de Swarm).
#
# Usage :
#   ./create2-vm.sh <VMID> [hostname] [flags...]
#
# Exemples :
#   ./create2-vm.sh 100 ag-test
#   ./create2-vm.sh 101 ag-docker --docker
#   ./create2-vm.sh 102 ag-mgr --initswarm
#   ./create2-vm.sh 103 ag-worker --joinswarm 102
#   ./create2-vm.sh 104 ag-mgr2 --joinswarm 102 --as-manager
#   STORAGE=local-lvm DISK_SIZE=50 ./create2-vm.sh 105 ag-data --initswarm
#
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --swarm)        SWARM_MODE=1 ;;
        --initswarm)    INIT_SWARM=1 ;;
        --joinswarm)    JOIN_SWARM=1; JOIN_MANAGER_CTID="${2:-}"; shift ;;
...
# Mutual exclusion : erreur si --initswarm ET --joinswarm
if [ "${INIT_SWARM}" -eq 1 ] && [ "${JOIN_SWARM}" -eq 1 ]; then
    fail "--initswarm et --joinswarm sont mutuellement exclusifs" \
         "Choisissez l'un OU l'autre : --initswarm pour creer un cluster, --joinswarm <MANAGER_CTID> pour rejoindre."
fi
...
# Resolution MANAGER_CTID -> manager_ip + token via le fichier produit par --initswarm
if [ "${JOIN_SWARM}" -eq 1 ]; then
    if [ -z "${JOIN_MANAGER_CTID}" ]; then
        fail "--joinswarm requiert un MANAGER_CTID en argument" \
             "Usage : $0 <VMID> [hostname] --joinswarm <MANAGER_CTID> [--as-manager]"
    fi

    TOKEN_FILE_LXC="/root/.ssh/lxc-keys/swarm-tokens-${JOIN_MANAGER_CTID}.json"
    TOKEN_FILE_VM="/root/.ssh/vm-keys/swarm-tokens-vm${JOIN_MANAGER_CTID}.json"

    if [ -f "${TOKEN_FILE_LXC}" ]; then
        JOIN_TOKEN_SOURCE="${TOKEN_FILE_LXC}"
    elif [ -f "${TOKEN_FILE_VM}" ]; then
        JOIN_TOKEN_SOURCE="${TOKEN_FILE_VM}"
    else
        fail "Aucun fichier de tokens trouve pour MANAGER_CTID=${JOIN_MANAGER_CTID}" \
             "Cherche : ${TOKEN_FILE_LXC} et ${TOKEN_FILE_VM}. Le manager doit avoir ete initialise au prealable avec --initswarm."
    fi

    JOIN_MANAGER_IP=$(grep -oP '"manager_ip"[[:space:]]*:[[:space:]]*"\K[^"]+' "${JOIN_TOKEN_SOURCE}" 2>/dev/null | head -1)
    if [ "${JOIN_AS_MANAGER}" -eq 1 ]; then
        JOIN_TOKEN=$(grep -oP '"manager_token"[[:space:]]*:[[:space:]]*"\K[^"]+' "${JOIN_TOKEN_SOURCE}" 2>/dev/null | head -1)
    else
        JOIN_TOKEN=$(grep -oP '"worker_token"[[:space:]]*:[[:space:]]*"\K[^"]+' "${JOIN_TOKEN_SOURCE}" 2>/dev/null | head -1)
    fi

    if [ -z "${JOIN_MANAGER_IP}" ] || [ -z "${JOIN_TOKEN}" ]; then
        fail "Impossible d'extraire manager_ip ou token depuis ${JOIN_TOKEN_SOURCE}" \
             "Verifiez que le fichier est un JSON valide produit par --initswarm."
    fi

    echo "  -> MANAGER_CTID=${JOIN_MANAGER_CTID} resolu via ${JOIN_TOKEN_SOURCE}"
    echo "     IP    : ${JOIN_MANAGER_IP}"
    echo "     Role  : $([ "${JOIN_AS_MANAGER}" -eq 1 ] && echo manager || echo worker)"
fi
...
if [ -z "${VMID}" ]; then
    cat << 'USAGE'
Usage : ./create2-vm.sh <VMID> [hostname] [flags...]
...
Flags :
  ...
  --initswarm                    Init un cluster Swarm (implique --docker + --swarm)
  --joinswarm <MANAGER_CTID>     Rejoint un cluster (implique --docker + --swarm)
                                 MANAGER_CTID = CTID LXC ou VMID du manager existant
                                 (manager_ip + token resolus auto via le fichier
                                  de tokens cree par --initswarm)
      --as-manager               Rejoindre comme manager (defaut : worker)

--initswarm et --joinswarm sont mutuellement exclusifs.
...
[ "${JOIN_SWARM}" -eq 1 ] && echo "  Join-Swarm : ACTIVE (manager_ctid=${JOIN_MANAGER_CTID}, ip=${JOIN_MANAGER_IP})"
echo "==========================================="
echo ""
...
if [ "${JOIN_SWARM}" -eq 1 ] && [ "${JOIN_SWARM_OK}" -eq 1 ]; then
    echo "  Swarm        : rejoint comme ${JOIN_NEW_ROLE} (manager_ctid=${JOIN_MANAGER_CTID}, ip=${JOIN_MANAGER_IP})"
fi
echo "==========================================="
echo ""
...
build_swarm_json() {
    ...
    if [ "${INIT_SWARM}" -eq 1 ]; then
        ...
        printf '"target_manager":{"ctid":"%s","ip":"%s","port":2377,"token_source":"%s"},' \
            "$(json_escape "${JOIN_MANAGER_CTID}")" \
            "$(json_escape "${JOIN_MANAGER_IP}")" \
            "$(json_escape "${JOIN_TOKEN_SOURCE}")"
